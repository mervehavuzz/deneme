...
# ─────────────────────────────────────────
# API & MODEL (LLAMA 3.1 & GÜÇLÜ PROMPT)
# ─────────────────────────────────────────
...
KRİTİK KURALLAR:
- Kesinlikle 'Hukuka aykırı eylem'i (zarar verme, çalma, izinsiz girme) bir meşru menfaat olarak tanımlama! Suç meşru menfaat olamaz.
- Başkasının e-postasına girmek 'Haberleşmenin Gizliliğini İhlal' (TCK 132) ve 'Verileri Hukuka Aykırı Ele Geçirme' (TCK 136) suçudur.
...
- Eğer bilgi eksikse varsayım yapma, 'verilen bilgiler ışığında' diyerek analizi sınırlandır."""

try:
    hf_token = st.secrets["HF_TOKEN"]
    # Muhakeme yeteneği daha yüksek olan Llama 3.1 8B modeli
...
def stream_response(prompt: str, placeholder) -> str:
    ...
    try:
        ...
        placeholder.error(f"⚠️ Yanıt oluşturulurken hata oluştu. Lütfen tekrar deneyin.")
        return "Hata nedeniyle yanıt oluşturulamadı."
